...
<head>
    ...
    <style>
        ...
        body.collapsed .tree-line {
            display: none !important;
        }

        body.collapsed #sidebar .menu-item-content {
            justify-content: center;
            padding-left: 0;
            padding-right: 0;
        }

        body.collapsed #sidebar .menu-icon {
            margin-right: 0;
        }
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.store('theme', {
                ...
                init() {
                    const dbSettings = @json(auth()->user()->theme_settings ?? null);
                    if (dbSettings) {
                        if (dbSettings.light) this.settings.light = {
                            ...this.settings.light,
                            ...dbSettings.light
                        };
                        if (dbSettings.dark) this.settings.dark = {
                            ...this.settings.dark,
                            ...dbSettings.dark
                        };
                ...
                async save() {
                    ...
                    try {
                        const token = document.querySelector('meta[name="csrf-token"]').getAttribute(
                            'content');
                        await axios.post("{{ route('admin.theme.update') }}", {
                            theme: this.settings
                        }, {
                            headers: {
                                'X-CSRF-TOKEN': token
                ...
                get css() {
                    ...
                    const shadowVal = this.settings.shadow ?
...
<body
    class="antialiased {{ App::getLocale() == 'km' ? 'font-khmer' : 'font-sans' }} bg-page-bg preload font-sans flex h-screen overflow-hidden text-gray-800 dark:text-gray-100 transition-colors duration-300">
    ...
    {{-- Backdrop សម្រាប់ Mobile --}}
    <div id="mobileBackdrop" class="fixed inset-0 bg-black/50 z-40 hidden md:hidden transition-opacity"></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const toggleBtn = document.getElementById('sidebarToggle'); // សម្រាប់ Desktop
            const mobileToggleBtn = document.getElementById(
            'mobileSidebarToggle'); // សម្រាប់ Mobile (ត្រូវបង្កើតក្នុង Header)
            const mobileBackdrop = document.getElementById('mobileBackdrop'); // Backdrop ថ្មី

            // --- ផ្នែក Desktop Collapse (កូដចាស់របស់អ្នក) ---
            const isCollapsed = localStorage.getItem('sidebar-collapsed') === 'true';
            if (isCollapsed && window.innerWidth >= 768) { // អនុវត្តតែលើកុំព្យូទ័រប៉ុណ្ណោះ
                ...
                toggleBtn.addEventListener('click', () => {
                    ...
                    if (sidebar) {
                        if (isNowCollapsed) {
                            sidebar.classList.remove('w-72');
                            sidebar.classList.add('w-20');
                            document.querySelectorAll('.arrow-icon').forEach(el => el.classList.remove(
                                'rotate-180'));
                        } else {
            ...
            // --- ផ្នែក Mobile Toggle (ថ្មី) ---
            function toggleMobileSidebar() {
                if (sidebar) {
                    // បើក/បិទ Sidebar ដោយប្ដូរ Class Translate
                    sidebar.classList.toggle('-translate-x-full');
                }
                if (mobileBackdrop) {
                    // បង្ហាញ/លាក់ Backdrop
                    mobileBackdrop.classList.toggle('hidden');
                }
            }

            // ចុចលើ Menu Icon លើទូរស័ព្ទ
            if (mobileToggleBtn) {
                mobileToggleBtn.addEventListener('click', toggleMobileSidebar);
            }

            // ចុចលើ Backdrop ដើម្បីបិទ Sidebar
            if (mobileBackdrop) {
                mobileBackdrop.addEventListener('click', toggleMobileSidebar);
            }
        });

        // ដូរពីរបៀបសរសេរធម្មតា មកជា window.toggleSubmenu វិញ
        window.toggleSubmenu = function(button) {
            // ឆែកមើលថាបើ Sidebar កំពុងបង្រួម (Collapsed) គឺមិនឲ្យបើក Submenu ទេ
