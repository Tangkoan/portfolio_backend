<div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-4">
    <h1 class="flex items-center gap-2 text-xl sm:text-2xl font-bold text-text-color">
        <i class="ri-briefcase-4-line"></i> {{ __('messages.experience_management') }}
    </h1>
    <div class="hidden md:flex items-center gap-2">
        ...
        >
            ...
            <span>{{ __('messages.add_experience') }}</span>
        </button>
    </div>
</div>
...
<div class="flex flex-col md:flex-row gap-3 mb-6">
    <div class="relative flex-1">
        ...
        <input 
            ...
            placeholder="{{ __('messages.search_experience') }}" 
        ...
        <div x-show="openCol" x-transition class="absolute right-0 mt-2 w-44 bg-card-bg border border-border-color rounded-xl shadow-xl z-50 p-2" x-cloak>
            <div class="space-y-1">
                <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-page-bg rounded cursor-pointer">
                    <input type="checkbox" x-model="showCols.company" class="rounded border-input-border text-primary focus:ring-primary">
                    <span class="text-sm text-text-color">{{ __('messages.company') }}</span>
                </label>
                <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-page-bg rounded cursor-pointer">
                    <input type="checkbox" x-model="showCols.duration" class="rounded border-input-border text-primary focus:ring-primary">
                    <span class="text-sm text-text-color">{{ __('messages.duration') }}</span>
                </label>
                <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-page-bg rounded cursor-pointer">
