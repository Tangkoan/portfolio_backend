<div class="flex flex-col gap-3">
    ...
    {{-- Select All Row --}}
    <div class="flex items-center justify-between px-2" x-show="projects?.length > 0" x-cloak>
        <label class="flex items-center gap-2 text-sm font-bold text-text-color select-none cursor-pointer">
            ...
            <span :class="{'opacity-50': {{ auth()->user()->cannot('projects-delete') ? 'true' : 'false' }}}">{{ __('messages.select_all') }}</span>
        </label>
        <span class="text-xs text-secondary"><span x-text="projects?.length"></span> {{ __('messages.items') }}</span>
    </div>
                ...
                <div x-show="showCols.link" class="mt-1">
                    <template x-if="item.url_project">
                        <a :href="item.url_project" target="_blank" class="text-blue-500 hover:text-blue-700 hover:underline flex items-center gap-1 text-sm bg-blue-50 px-3 py-1.5 rounded-lg inline-flex w-fit">
                            <i class="ri-external-link-line"></i> {{ __('messages.visit_project') }}
                        </a>
                    </template>
                </div>
    ...
    {{-- No data --}}
    <div x-show="projects?.length === 0" x-cloak class="text-center py-10 text-secondary bg-card-bg rounded-xl border border-dashed border-border-color">
