        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-page-bg/50 border-b border-border-color text-text-color text-sm uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4 font-bold w-24 text-center" x-show="showCols.status">Status</th>
                    <th class="px-6 py-4 font-bold text-right w-28">Actions</th>
                </tr>
            </thead>
                ...
                <template x-for="item in items" :key="item.id">
                    <tr class="hover:bg-page-bg/30 transition-colors">
                        ...
                        <td class="px-6 py-4" x-show="showCols.image">
                            <div class="h-10 w-10 rounded-full bg-gray-100 overflow-hidden border border-border-color">
                                <template x-if="item.image"><img :src="'/storage/' + item.image" class="w-full h-full object-cover"></template>
                                <template x-if="!item.image"><div class="w-full h-full flex items-center justify-center text-secondary"><i class="ri-user-line"></i></div></template>
                            </div>
                        </td>

                        <td class="px-6 py-4 font-bold text-text-color whitespace-nowrap" x-text="item.name"></td>
                        
                        <td class="px-6 py-4 text-secondary text-sm" x-show="showCols.description">
