<div class="flex flex-col gap-3">
    <div class="flex items-center justify-between px-2" x-show="items?.length > 0" x-cloak>
        <label class="flex items-center gap-2 text-sm font-bold text-text-color select-none cursor-pointer">
            {{-- Checkbox ជ្រើសរើសទាំងអស់ (បិទបើគ្មានសិទ្ធិ Delete) --}}
            <input type="checkbox" @change="toggleSelectAll()" x-model="selectAll" 
                @cannot('about_me-delete') disabled @endcannot
            ...
            <span :class="{'opacity-50': {{ auth()->user()->cannot('about_me-delete') ? 'true' : 'false' }}}">
                {{ __('messages.select_all') }}
            </span>
        </label>
        <span class="text-xs text-secondary"><span x-text="items?.length"></span> {{ __('messages.items') }}</span>
    </div>
    ...
    <template x-for="item in items" :key="'mobile-' + item.id">
        <div class="bg-card-bg p-4 rounded-2xl shadow-sm border border-border-color relative overflow-hidden transition-all duration-200"
            ...
            {{-- Checkbox ជ្រើសរើសមួយៗ (បិទបើគ្មានសិទ្ធិ Delete) --}}
            ...
            <div class="flex flex-col gap-3 pl-8"> 
                ...
                <div class="flex items-center justify-between mt-2 pt-3 border-t border-dashed border-border-color">
                    ...
                    <div class="flex items-center gap-2" x-show="showCols.status">
                        ...
                        {{-- ប្តូរពាក្យ Active/Inactive ទៅជាភាសា --}}
                        <span class="text-[10px] font-bold uppercase" :class="item.status ? 'text-green-600' : 'text-gray-400'" 
                              x-text="item.status ? '{{ __('messages.status_active') }}' : '{{ __('messages.status_inactive') }}'">
                        </span>
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <button 
                            ...
                            @else
                                disabled
                            @endcan
                            class="h-8 w-8 rounded-full flex items-center justify-center bg-blue-50 text-blue-600 border border-blue-100 hover:bg-blue-100 active:scale-95 transition-transform disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 disabled:hover:bg-blue-50"
                        >
                        ...
                        <button 
                            ...
                            @else
                                disabled
                            @endcan
                            class="h-8 w-8 rounded-full flex items-center justify-center bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 active:scale-95 transition-transform disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 disabled:hover:bg-red-50"
                        >
    ...
    <div x-show="items?.length === 0" x-cloak class="text-center py-10 text-secondary bg-card-bg rounded-xl border border-dashed border-border-color">
