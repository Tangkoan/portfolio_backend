...
<div class="flex flex-col md:flex-row gap-3 mb-6">
    <div class="relative flex-1">
        ...
        <input type="text" x-model="search" @keyup.debounce.500ms="fetchData()" placeholder="Search profile..." class="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl border border-input-border bg-card-bg text-text-color shadow-sm focus:ring-primary outline-none">
    </div>
    ...
    <div class="relative shrink-0">
        ...
        <div x-show="openCol" x-transition class="absolute right-0 mt-2 w-44 bg-card-bg border border-border-color rounded-xl shadow-xl z-50 p-2">
            <div class="space-y-1">
                ...
                <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-page-bg rounded cursor-pointer">
                    <input type="checkbox" x-model="showCols.description" class="rounded border-input-border text-primary focus:ring-primary"><span class="text-sm text-text-color">Description</span>
                </label>
                <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-page-bg rounded cursor-pointer">
