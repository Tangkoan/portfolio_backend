<div class="flex flex-col gap-3">
    ...
    {{-- Select All Row --}}
    <div class="flex items-center justify-between px-2" x-show="technologies?.length > 0" x-cloak>
        <label class="flex items-center gap-2 text-sm font-bold text-text-color select-none cursor-pointer">
            ...
            <span :class="{'opacity-50': {{ auth()->user()->cannot('technologies-delete') ? 'true' : 'false' }}}">{{ __('messages.select_all') }}</span>
        </label>
    ...
    <template x-for="tech in technologies" :key="'mobile-' + tech.id">
        <div class="bg-card-bg p-3 rounded-2xl shadow-sm border border-border-color relative overflow-hidden transition-all duration-200"
            ...
            <input type="checkbox" :value="tech.id" x-model="selectedIds" 
                   @cannot('technologies-delete') disabled @endcannot
                   class="absolute top-3 left-3 z-20 rounded-md border-gray-300 text-primary focus:ring-primary h-5 w-5 shadow-sm bg-white disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-200">
                    ...
                    <div class="flex items-center justify-between mt-3 pt-2 border-t border-dashed border-border-color" x-show="showCols.status">
                        <div class="flex items-center gap-2">
                            <button 
                                ...
                                :class="tech.status ? 'bg-green-500' : 'bg-gray-300'"
                            >
                            ...
                            <span class="text-[10px] font-bold uppercase" :class="tech.status ? 'text-green-600' : 'text-gray-400'" 
                                  x-text="tech.status ? '{{ __('messages.status_active') }}' : '{{ __('messages.status_inactive') }}'"></span>
                        </div>
    ...
    {{-- No data --}}
    <div x-show="technologies?.length === 0" x-cloak class="text-center py-10 text-secondary bg-card-bg rounded-xl border border-dashed border-border-color">
