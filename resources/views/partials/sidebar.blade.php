...
{{--                 PORTFOLIO SECTION               --}}
{{-- ============================================= --}}
@canany(['projects-list', 'technologies-list', 'experiences-list', 'socials-list', 'tools-list', 'about_me-list', 'certificates-list'])

@php 
    // បានបន្ថែម route ថ្មីៗចូលទៅក្នុង Logic ដើម្បីបើក/បិទ Menu
    $isPortfolioActive = request()->routeIs('admin.projects.*') || 
                         ...
                         request()->routeIs('admin.about_me.*') ||
...
<div class="group relative">
    ...
    <div class="submenu {{ $isPortfolioActive ? '' : 'hidden' }} transition-all duration-300">
        ...
        <ul class="space-y-1 mt-1">
            ...
            @can('about_me-list')
            <li>
                <a href="{{ route('admin.about_me.index') }}" 
                   class="sidebar-item relative flex items-center py-2.5 rounded-lg text-sm transition-all duration-200 pl-12 pr-4
                          {{ request()->routeIs('admin.about_me.*') ? 'text-primary font-bold' : 'opacity-80' }}">
                    <span class="tree-line absolute left-[22px] top-1/2 -translate-y-1/2 w-2 h-2 rounded-full border-2 border-sidebar-bg 
                                 {{ request()->routeIs('admin.about_me.*') ? 'bg-primary' : 'bg-gray-400' }}"></span>
                    <span>{{ __('sidebar.about_me') ?? 'About Me' }}</span>
                </a>
            </li>
            @endcan
            ...
            <li>
                <a href="{{ route('admin.tools.index') }}" 
                   class="sidebar-item relative flex items-center py-2.5 rounded-lg text-sm transition-all duration-200 pl-12 pr-4
                          {{ request()->routeIs('admin.tools.*') ? 'text-primary font-bold' : 'opacity-80' }}">
                    <span class="tree-line absolute left-[22px] top-1/2 -translate-y-1/2 w-2 h-2 rounded-full border-2 border-sidebar-bg 
                                 {{ request()->routeIs('admin.tools.*') ? 'bg-primary' : 'bg-gray-400' }}"></span>
                    <span>{{ __('sidebar.tools') ?? 'Tools' }}</span>
                </a>
            </li>
            @endcan
            ...
            <li>
                <a href="{{ route('admin.certificates.index') }}" 
                   class="sidebar-item relative flex items-center py-2.5 rounded-lg text-sm transition-all duration-200 pl-12 pr-4
                          {{ request()->routeIs('admin.certificates.*') ? 'text-primary font-bold' : 'opacity-80' }}">
                    <span class="tree-line absolute left-[22px] top-1/2 -translate-y-1/2 w-2 h-2 rounded-full border-2 border-sidebar-bg 
                                 {{ request()->routeIs('admin.certificates.*') ? 'bg-primary' : 'bg-gray-400' }}"></span>
                    <span>{{ __('sidebar.certificates') ?? 'Certificates' }}</span>
                </a>
            </li>
            @endcan
